...
float gravity = -0.05; // скорость изменения координаты Y
...
void drawMap(Hero *obj)
{
    ...
    for (int i=0; i<H; i++) //чтение массива по вертикали
    {
        for (int j=0; j<W; j++) // чтение массива по горизонтали
        {
            if (TileMap[i][j]=='A')
            {
                renderAreaTexture(&tex_Blocks,j*tsize,i*tsize,40,80,40,40,0,0,true);
            }
            if (TileMap[i][j]=='B')
            {
                renderAreaTexture(&tex_Blocks,j*tsize,i*tsize,0,80,40,40,1,0,true);
            }
